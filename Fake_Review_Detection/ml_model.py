...
def calculate(asin_number):
    ...
    col = db["product"]
    col2 = db["feedback"]
    x = col.find_one({'asin_number': asin_number})
    y = col2.find({'asin_number': asin_number})

    def feedback_protocol(x,y,impact,model_score):
        for i in y:
            if x["asin_number"] == i["asin_number"]:
                pos = i["positive"]
                neg = i["negative"]
                if neg > pos:
                    diff = neg - pos
    ...
    model_score = feedback_protocol(x,y,impact,model_score)
